...
1. Reads the time-series dataset.
2. Parses timestamps.
3. Sorts observations chronologically.
4. Selects load and weather features.
5. Handles missing values.
6. Scales model inputs.
7. Builds rolling 24-hour sequences.
8. Splits data chronologically.
9. Trains the LSTM.
10. Evaluates predictions using forecasting error metrics.
11. Saves the model and scalers.
12. Generates an actual-vs-predicted plot.
...
**India Electricity Load Forecasting using LSTM**

- Built an LSTM-based time-series forecasting pipeline to predict hourly electricity load from 24-hour historical sequences using load, temperature, and humidity features.
- Preprocessed chronological time-series data, generated rolling input sequences, scaled features, and trained a stacked 64/32-unit LSTM model with Adam optimization and early stopping.
- Evaluated forecasting performance using MAE, RMSE, and MAPE and visualized actual-versus-predicted load to assess model behavior.
- Structured the project for reproducible training and inference with saved model and preprocessing artifacts.

## Interview One-Liner

> "I built an LSTM-based time-series forecasting system that uses the previous 24 hours of electricity load along with temperature and humidity to predict the next hourly load value, then evaluates the forecasts using MAE, RMSE, and MAPE."

## Interview Warning

Be honest about the dataset. If asked whether the weather data is real, say:

> "The weather variables in my original dataset are synthetic. I used them to demonstrate a multivariate forecasting pipeline. A production version would replace them with validated real weather observations."
